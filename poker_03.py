...
def allmax(iterable, key=None):
    #why call iterable, so that it is a generic function not tied to poker
    "Return a list of all items equal to the max of the iterable."
    # Your code here.
    result, maxval = [], None
    key = key or (lambda x: x) # if user sends in function
    for x in iterable:
        xval = key(x)
...
def my_allmax(iterable, key=None):
    #why call iterable, so that it is a generic function not tied to poker
    "Return a list of all items equal to the max of the iterable."
    # Your code here.
    return_value = []
    key = key or (lambda x: x) # if user sends in function
    max_x = max(iterable, key=key)
    max_val = key(max_x)
    for x in iterable:
        if key(x) == max_val:
...
def test():
    ...
    # fh playing against fh returns a tie.
